...
class AISDNController:
    ...
    def simulate_traffic(self, src_ip, dst_ip, src_port, dst_port, protocol, packet_size=1500):
        ...
        if self.flow_stats[flow_key]['packet_count'] == 0:
            # Classify traffic
            traffic_type = self._classify_traffic(dst_port, protocol, packet_size)
...
def main():
    ...
    # Simulate different traffic types with varied requirements
    traffic_flows = [
        ('10.0.0.1', '10.0.0.3', 12345, 80, 'TCP', 1200),      # HTTP
        ('10.0.0.2', '10.0.0.4', 54321, 554, 'TCP', 1400),     # Video
        ('10.0.0.3', '10.0.0.5', 16384, 5060, 'UDP', 160),     # VoIP
        ('10.0.0.4', '10.0.0.6', 8765, 21, 'TCP', 1500),       # FTP
        ('10.0.0.5', '10.0.0.1', 27015, 27015, 'UDP', 200),    # Gaming
    ]
    
    print("🌐 Simulating AI-Powered Network Traffic...")
    print("🤖 Making intelligent routing decisions...")
    print("Press Ctrl+C to stop\n")
    
    try:
        packet_count = 0
        while True:
            # Randomly select a flow to generate traffic
            ...
            time.sleep(0.8)  # Generate packet every 0.8 seconds
            
    except KeyboardInterrupt:
        print("\n🛑 Stopping AI simulation...")
        controller.show_statistics()
        print("✅ AI-Powered Simulation complete!")
        print("🎯 Network optimized with intelligent routing!")
